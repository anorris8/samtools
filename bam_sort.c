                    ...
                    int flag, const char *reg)
{
    ...
    if (flag & MERGE_UNCOMP) fpout = strcmp(out, "-")? bam_open(out, "wu") : bam_dopen(_fileno(stdout), "wu");
    else if (flag & MERGE_LEVEL1) fpout = strcmp(out, "-")? bam_open(out, "w1") : bam_dopen(_fileno(stdout), "w1");
    else fpout = strcmp(out, "-")? bam_open(out, "w") : bam_dopen(_fileno(stdout), "w");
    if (fpout == 0) {
...
static void sort_blocks(int n, int k, bam1_p *buf, const char *prefix, const bam_header_t *h, int is_stdout)
{
    ...
    fp = is_stdout? bam_dopen(_fileno(stdout), mode) : bam_open(name, mode);
    if (fp == 0) {
...
void bam_sort_core_ext(int is_by_qname, const char *fn, const char *prefix, size_t max_mem, int is_stdout)
{
    ...
    n = k = 0; mem = 0;
    fp = strcmp(fn, "-")? bam_open(fn, "r") : bam_dopen(_fileno(stdin), "r");
    if (fp == 0) {
...
int bam_sort(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "nom:")) >= 0) {
        ...
        case 'n': is_by_qname = 1; break;
        case 'm': max_mem = _atoi64(optarg); break;
        }
    }
    if (optind + 2 > argc) {
