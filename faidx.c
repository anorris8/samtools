...
#ifdef _WIN32
        fprintf(fp, "%s\t%I64d\t%I64u\t%d\t%d\n", fai->name[i], x.len, x.offset, (int)x.line_blen, (int)x.line_len); //dong code, verify
#else
...
faidx_t *fai_read(FILE *fp)
{
...
#ifdef _WIN32
    uint64_t offset; //dong code, verify
#else
    long long offset;
#endif
    ...
    while (!feof(fp) && fgets(buf, 0x10000, fp)) {
        ...
        *p = 0; ++p;
#ifdef _WIN32
        sscanf(p, "%d%I64u%d%d", &len, &offset, &line_blen, &line_len); //dong code, verify
#else
